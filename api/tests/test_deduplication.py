...
async def _make_job_and_completed_run(content_hash: str | None = None):
...
async def test_result_consumer_dedup_same_content_short_circuits():
    """Second run with identical content: diff_detected=False, history/ deleted, no LLM dispatch."""
    content = b"<html>same page</html>"
    content_hash = xxhash.xxh64(content).hexdigest()[:16]

    user_id, job_id, _ = await _make_job_and_completed_run(content_hash=content_hash)
