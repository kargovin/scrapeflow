...
| **P3b** ⚠️ **NEXT** | **UF-003 — MinIO write-path failures handled inconsistently** | Surfaced closing UF-001. Three behaviours, none right: (3a) **playwright + Go workers ack on a MinIO write fault** → the Q5 ack-on-failure bug, still unfixed on 2 of 3 workers; a transient blip permanently fails a job after the expensive render/LLM call is done. Q5's fix (`llm-worker/worker/errors.py` `_TRANSIENT_S3_CODES`) landed on the LLM worker **only**. (3b) `result_consumer` swallows MinIO errors with **no log line** (`_compute_content_hash`, `stat_minio_size`) → silent dedup degradation + permanent quota under-count. **3a survives Temporal as domain knowledge** — the transient/terminal S3 split is the same classification §3 says must port into the activity `RetryPolicy`; propagating it to all 3 workers now fixes a live bug *and* consolidates it before the one-time port. **3b:** `result_consumer.py` is deleted by the migration — bare `logger.warning` is the ceiling; the `stat_minio_size` line (money-adjacent quota state, in surviving `storage.py`) is the only one worth adding. Full detail in `usage-findings.md` → UF-003. | S (3a) / XS (3b) |
