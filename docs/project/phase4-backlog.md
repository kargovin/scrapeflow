...
>
> **Queue: P6 → P9 → P8 → P7 + BUG-007 together.**
...
| **P9** 🔴 **OPEN** | **BUG-011 — stale-pending recovery silently skips every batch run.** `_recover_stale_pending` (`core/scheduler.py`) is the only guard against crash-after-commit-before-publish. Its `SELECT` has **no lane filter**, so batch runs are selected, then dropped by `db.get(Job, run.job_id)` returning `None` — every tick, with no log line, under a comment asserting the case is impossible. Recovery covers the job and scheduled lanes and silently excludes batch. | **Filed 2026-09-03 tracing P6's scope against ADR-011.** Not part of BUG-005: ADR-011 removes the *cause* (messages stop being dropped) and leaves the net holed, because this is not a parsing problem — the recovery function has one payload shape. Latent after P6, live the first time a batch dispatch is genuinely lost, which is what the function exists for. **Fix is small — everything it needs is persisted** (`batch_items.url`; `batches.output_format/engine/respect_robots/llm_config`), so it routes by whichever FK is set, like the result consumer. ⚠️ **P6 already edits this function** (it builds a payload containing `job_id`), so one pass is materially cheaper than two. `open-bugs.md` → BUG-011 | S |
...
1. **Pre-Phase 4 (§1)** — P1 → P5 are done. **Four left: P6 → P9 → P8 → P7 + BUG-007.**
