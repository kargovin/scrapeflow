...
- **Pre-Phase 4 queue:** Q6 LLM `ack_wait` ✅ **closed in prod** → Q5 cold starts ✅ **closed in
  ...
  new `/health/deps` endpoint, split from the `/health/ready` probe) → **UF-003 (P3b, IN PROGRESS,
  unpushed)** inconsistent MinIO write-path failure handling: playwright 3a ✅ `2432be7` + LLM
  aiohttp gap ✅ `6ad95e3`, **remaining = Go worker 3a → 3b `result_consumer` log lines** ←
  **resume here** → Q1–Q4 close-out → BUG-002 Dependabot crit+highs (8 crit / 13 high, drifting
  up). New: **BUG-004** (screenshots orphaned on every path) filed to §4. **⚠️ the last 4 commits
  on `develop` are unpushed and undeployed.**
