...
| Playwright NATS consumer `ack_wait` | Explicit `ConsumerConfig(ack_wait=120)` on the `python-playwright-worker` pull consumer + `msg.in_progress()` heartbeat every 30s during a job | JetStream default `ack_wait=30s` < a headed-Chrome scrape (~37s) → redelivery mid-job, no-op late `ack()`, and with `max_deliver=-1` an **infinite re-scrape loop** (Q6, fired in prod 2026-07-03). Heartbeat covers jobs longer than `ack_wait`. **JetStream won't change `ack_wait` on an existing durable consumer — update/recreate out-of-band** (`add_consumer` with modified config; this nats-py has no `update_consumer`). **Same latent bug likely on the LLM worker consumer — audit it** |
