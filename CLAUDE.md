...
- **Pre-Phase 4 queue:** Q6 LLM `ack_wait` ✅ **closed in prod** → Q5 cold starts ✅ code done
  (**`e1fde0d` unpushed; needs a consumer recreate for `max_deliver`**) → BUG-003 bot walls
  → UF-001 MinIO health check → Q1–Q4 close-out → BUG-002 Dependabot crit+highs.
