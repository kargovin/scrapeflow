...
- **Pre-Phase 4 queue:** Q6 LLM `ack_wait` ✅ **closed in prod** → Q5 cold starts ✅ **closed in
  prod** (`fbcf254`; consumer recreated, `max_deliver: 3` verified) → BUG-003 bot walls ✅ **code
  merged** (`8168760`) + poisoned baselines cleaned in prod, ⚠️ **image not yet deployed** →
  **next: UF-001 MinIO health check** → Q1–Q4 close-out → BUG-002 Dependabot crit+highs (now 8
  crit / 13 high, drifting up). New: **BUG-004** (screenshots orphaned on every path) filed to §4.
