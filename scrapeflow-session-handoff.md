...
- **Phase 3 in progress — Steps 1–13 done, Step 14 is next**
...
**Step 14 — Go HTTP worker: schema_version 2** (`docs/project/PHASE3_BACKLOG.md` §Step 14):
- Update worker message struct to `schema_version: 2` with nested `Credentials`, `Options`, `CrawlContext` sub-objects (ADR-004)
- Add proxy routing via `http.Transport`
- Add robots.txt enforcement (fetch direct, never via proxy; disallowed → fail run with `robots_txt_disallowed`)
