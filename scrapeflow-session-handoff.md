...
4. **BUG-014 — `DELETE /admin/users/{id}` 500s for any user with a batch or a crawl.** Filed
   2026-09-18, **not fixed, owner's to sequence.** Small and self-contained; here is the whole
   pick-up so no re-derivation is needed:
   - **Cause:** `crawls.user_id` (`api/app/models/crawl.py`) and `batches.user_id`
     (`api/app/models/batch.py`) are the only two of `users`' seven referrers with no
     `ondelete`; the `User` model cascades only `api_keys` and `jobs`. Postgres refuses the
     delete on `crawls_user_id_fkey` / `batches_user_id_fkey`. Verified in a rolled-back
     transaction.
   - **Fix:** `ondelete="CASCADE"` on both FKs → one autogenerated migration (⚠️ **delete the
     `drop_index("idx_webhook_deliveries_dedup")` op it will also emit** — the standing
     autogenerate false positive; and mind the hot-reload trap: the API applies the file on
     save) → one test in `test_admin.py`: delete a user who owns a crawl *and* a batch, assert
     204 and both parent rows gone. Keep the endpoint's release-before-delete order exactly as
     it is — a cascade that dropped `crawl_pages`/`batch_items`→`job_runs` first would take
     `storage_objects` rows with it for objects still on disk, the orphan class P8 closed.
   - **Sequencing call:** it is a migration, so it either rides the queue's single release as a
     **third** revision (do it before the `main` fast-forward) or ships as its own push later.
     Nothing depends on it; the interim state it leaves (files gone from MinIO, ledger rows
     stay) self-heals on the first delete that succeeds. Writeup: `open-bugs.md` → BUG-014;
     backlog §4 row.
