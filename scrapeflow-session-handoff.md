...
- **Phase 3 in progress — Steps 1–22 done, Step 23 is next**
- 190 API tests passing (deterministic — first-run clean); 69 playwright-worker tests passing
...
**Step 23 — PRD-007: coordinator service** (`docs/project/PHASE3_BACKLOG.md` §Step 23):
- New `coordinator/` service (Python) — BFS dispatch loop + NATS result subscriber
- Polls `crawl_queue WHERE status = 'pending'` in batches; creates `crawl_pages` row, dispatches fat NATS message with `crawl_context` sub-object
- Result handler: processes `scrapeflow.jobs.result` messages where `crawl_context IS NOT NULL`; extracts links from MinIO HTML via BeautifulSoup; inserts new URLs into `crawl_queue` (UNIQUE constraint handles dedup silently)
- Crawl completion detection: when `pending` queue count reaches 0 and all dispatched are terminal → set `crawls.status = 'completed'`, fire `crawl.completed` webhook
- Startup re-enqueue: stalled `dispatched` items (`dispatched_at < NOW() - 10 min`) reset to `pending`
- Needs separate NATS durable consumer (not shared with API result consumer)
- Add `coordinator` service to `docker-compose.yml`
