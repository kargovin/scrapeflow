...
- **Phase 3 in progress — Steps 1–12 done, Step 13 is next**
...
**Step 13 — Migration 3.10** (`docs/project/PHASE3_BACKLOG.md` §Step 13):
- Add `UniqueConstraint('user_id', 'name', name='uq_api_keys_user_name')` to `api_keys`
- Update `POST /users/api-keys` to catch `IntegrityError` and return `409 Conflict`
- Autogenerate handles the constraint after adding it to the `ApiKey` model
