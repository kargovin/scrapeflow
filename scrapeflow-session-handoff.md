...
- **Phase 3 in progress — Steps 1–8 done, Step 9 is next**
...
**Step 9 — Migration 3.6** (`docs/project/PHASE3_BACKLOG.md` §Step 9) ⚠ hand-written:
- Drop NOT NULL on `job_runs.job_id`
- Add `batch_item_id UUID NULL FK → batch_items`
- Add `content_hash VARCHAR(16) NULL`
- Hand-write CHECK constraint `(job_id IS NOT NULL) != (batch_item_id IS NOT NULL)` (autogenerate skips CHECK constraints)
- Partial index `WHERE batch_item_id IS NOT NULL`

Steps 10–13 are the remaining schema migrations. Step 12 is hand-written (DB trigger).
