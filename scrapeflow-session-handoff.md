...
- **Phase 3 in progress — Steps 1–21 done, Step 22 is next**
- 177 API tests passing (deterministic — first-run clean); 69 playwright-worker tests passing
...
**Step 22 — PRD-007: site crawl — API routes** (`docs/project/PHASE3_BACKLOG.md` §Step 22):
- New `POST /crawls`, `GET /crawls/{id}`, `GET /crawls/{id}/pages`, `DELETE /crawls/{id}`
- One-active-crawl-per-domain enforcement (409 with `crawl_id` if found)
- `POST /crawls` creates `crawls` row + inserts seed URL into `crawl_queue`; API does NOT dispatch — coordinator picks it up
- `DELETE /crawls/{id}` sets status `cancelled`; marks pending `crawl_queue` rows `skipped`
- Depends on Migration 3.7 (`crawls`, `crawl_pages`, `crawl_queue` tables — Step 10)
