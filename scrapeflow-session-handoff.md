...
- **Phase 3 in progress — Steps 1–20 done, Step 21 is next**
- 165 API tests passing (deterministic — first-run clean); 69 playwright-worker tests passing
...
**Step 21 — PRD-006: batch scraping** (`docs/project/PHASE3_BACKLOG.md` §Step 21):
- New `POST /batch` endpoint + `GET /batch/{id}` + `GET /batch/{id}/items`
- `BatchCreate` schema: `url_list: list[AnyHttpUrl]` + all `_MutableJobFields` — each URL becomes a `batch_items` row
- Result consumer gains a batch branch: when `batch_item_id` FK is set (not `job_id`), update `batch_items` row + atomic progress counter on `batches`; fire `batch.completed` webhook when `completed + failed == total`
- Respects `webhook_events` filter from Step 20
