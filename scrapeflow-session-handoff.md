...
- **Phase 3 in progress — Steps 1–19 done, Step 20 is next**
- 160 API tests passing (deterministic — first-run clean); 69 playwright-worker tests passing
...
**Step 20 — PRD-013: webhook event filter** (`docs/project/PHASE3_BACKLOG.md` §Step 20):
- `webhook_events: list[str] | None = None` on `JobCreate`/`JobPatch`/`JobResponse`; stored in `jobs.webhook_events TEXT[]` (column already exists from Migration 3.3)
- Validation: only known event names allowed — `job.completed`, `job.failed`, `crawl.completed`, `batch.completed`
- In result consumer: before creating a `webhook_deliveries` row, check `if job.webhook_events and event_name not in job.webhook_events: return`
- Both webhook call sites in result consumer (non-LLM completed path and LLM completed path) need the filter
